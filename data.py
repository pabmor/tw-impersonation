...
class Db:
    ...
    def _create_tables(self):
        self.cursor.execute('''CREATE TABLE IF NOT EXISTS {}
                        (id int PRIMARY KEY, user_id int, date string, content string)'''.format(Db._tableNames["statuses"]))
    ...
    def get_status(self, status_id, user_id):
        transaction = self.cursor.execute("SELECT * FROM {} WHERE user_id = ? AND id = ?"
            .format(Db._tableNames["statuses"]), (user_id, status_id))
